...
export function formatReceiptForSheet(record: {
  ...
  const taxInclusive = typeof record.taxInclusive === 'number' ? record.taxInclusive : Math.round((untaxedTotal + taxAmount) * 100) / 100;
...
export async function syncDirectToGoogleSheet(
  ...
  const response = await fetch('/api/proxy-direct-sheets', {
    ...
    body: JSON.stringify({
      ...
      values: rows,
      range: range,
      startRowIndex: record.startRowIndex,
      endRowIndex: record.endRowIndex
    })
  });
...
export function parseSheetRowsToReceipts(rows: any[][]): any[] {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    let date = row[0] ? String(row[0]).trim() : '';
    if (date && (date.includes('T') || date.includes(' ') || date.endsWith('Z'))) {
