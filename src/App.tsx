...
export default function App() {
  ...
  // Inbound & Payment process flow tracking states
  const [trackingStatuses, setTrackingStatuses] = useState<Record<string, { po?: boolean; request_payment?: boolean; paid?: boolean }>>(() => {
    try {
      const saved = localStorage.getItem('receipt_tracking_statuses_v2');
      if (saved) return JSON.parse(saved);
      // fallback and migrate from v1 (mutually exclusive) to v2
      const legacySaved = localStorage.getItem('receipt_tracking_statuses_v1');
      if (legacySaved) {
        const parsed = JSON.parse(legacySaved);
        const migrated: Record<string, { po?: boolean; request_payment?: boolean; paid?: boolean }> = {};
        Object.entries(parsed).forEach(([key, val]) => {
          if (val === 'po' || val === 'request_payment' || val === 'paid') {
            migrated[key] = { [val]: true };
          }
        });
        return migrated;
      }
  ...
  const handleUploadToGoogleSheets = async () => {
    ...
    try {
      if (sheetConfig.syncMode === 'appsScript') {
        await syncViaAppsScript(sheetConfig, finalizedDraft);
        const actionStr = isUpdateMode 
          ? `成功在原位置修改/更新第 ${activeDraft.startRowIndex}~${activeDraft.endRowIndex} 行数据！` 
          : `成功自动追加 ${activeDraft.items.length} 个入库货件到 Google Sheet！`;
        showToast(`⚡ ${actionStr}`, 'success');
        addLocalSyncLog(
          activeDraft.supplier, 
          activeDraft.items.length, 
          'success', 
          isUpdateMode 
            ? `成功覆盖更新云表中第 ${activeDraft.startRowIndex}~${activeDraft.endRowIndex} 行的数据流水` 
            : `成功向子表「${sheetConfig.sheetName || 'Sheet1'}」追加 ${activeDraft.items.length} 行数据`
        );
      } else {
        if (!googleAccessToken) throw new Error("授权缺失");
        await syncDirectToGoogleSheet(sheetConfig, googleAccessToken, finalizedDraft);
        const actionStr = isUpdateMode 
          ? `成功在原位置直接更新第 ${activeDraft.startRowIndex}~${activeDraft.endRowIndex} 行数据！` 
          : `成功直接 API 追加商品！`;
        showToast(`⚡ ${actionStr}`, 'success');
        addLocalSyncLog(
          activeDraft.supplier, 
          ...
          'success', 
          isUpdateMode 
            ? `通过 Google API 直接在原行数 A${activeDraft.startRowIndex}:L${activeDraft.endRowIndex} 覆盖修改成功` 
            : `通过 Google API 写入 ${activeDraft.items.length} 个商品`
        );
      }
      
      // Save locally as 'completed'
      saveToLocalLedger(finalizedDraft, true);
      showToast(isUpdateMode ? "物料数据原记录已成功修改同步。您可以继续核对下一张单据" : "物料数据已登记备份至云端。您可以继续核对下一张单据", "success");
      ...
      saveToLocalLedger(finalizedDraft, false);
      showToast(`云同步失败：已安全存盘至「下方本地账本」，可在排查网络后一键重试。`, 'error');
      addLocalSyncLog(activeDraft.supplier, activeDraft.items.length, 'failed', `云写入/修改失败: ${err.message || '450/401/403'}. 已在浏览器安全暂存盘。`);
  ...
  try {
    ...
    var startRowIndex = payload.startRowIndex;
    var endRowIndex = payload.endRowIndex;
    ...
    if (startRowIndex && endRowIndex) {
      ...
      return ContentService.createTextOutput(JSON.stringify({ success: true, updated: true, count: rows.length }))
        .setMimeType(ContentService.MimeType.JSON);
    }
...
function doGet(e) {
  ...
  // Dynamic calculated/overridden taxes of current draft
  const draftUntaxedTotal = activeDraft ? activeDraft.totalAmount : 0;
  const draftTax = activeDraft && typeof activeDraft.taxAmount === 'number'
    ? activeDraft.taxAmount
    : Math.round((draftUntaxedTotal * 0.07) * 100) / 100;
  const draftTaxInclusive = activeDraft && typeof activeDraft.taxInclusive === 'number'
    ? activeDraft.taxInclusive
    : Math.round((draftUntaxedTotal + draftTax) * 100) / 100;

  const handleDraftTotalAmountChange = (val: number) => {
    if (!activeDraft) return;
    setActiveDraft({
      ...activeDraft,
      totalAmount: val
  ...
  const handleDraftTaxAmountChange = (val: number) => {
    if (!activeDraft) return;
    setActiveDraft({
      ...activeDraft,
      taxAmount: val
  ...
  const handleDraftTaxInclusiveChange = (val: number) => {
    if (!activeDraft) return;
    setActiveDraft({
      ...activeDraft,
      taxInclusive: val
  ...
  const totalInvoicesCount = sheetReceipts.length;
  const totalUntaxedSum = sheetReceipts.reduce((acc, curr) => acc + (curr.totalAmount || 0), 0);
  const totalTaxSum = sheetReceipts.reduce((acc, curr) => {
    const tax = typeof curr.taxAmount === 'number' ? curr.taxAmount : (curr.totalAmount || 0) * 0.07;
    return acc + tax;
  }, 0);
  const totalInclusiveSum = sheetReceipts.reduce((acc, curr) => {
    const incl = typeof curr.taxInclusive === 'number' ? curr.taxInclusive : (curr.totalAmount || 0) * 1.07;
    return acc + incl;
  }, 0);
  const uniqueSuppliersCount = new Set(sheetReceipts.map(r => r.supplier).filter(Boolean)).size;
  const totalItemsProcessed = sheetReceipts.reduce((acc, curr) => acc + (curr.items?.length || 0), 0);

  const pendingPoCount = sheetReceipts.filter(r => !trackingStatuses[r.id]?.po).length;
  const pendingRequestPaymentCount = sheetReceipts.filter(r => !trackingStatuses[r.id]?.request_payment).length;
  const pendingPaidCount = sheetReceipts.filter(r => !trackingStatuses[r.id]?.paid).length;

  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans relative antialiased text-gray-850">
      ...
      <header className="bg-white border-b border-slate-200/85 sticky top-0 z-40">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="hidden md:flex items-center gap-2">
            {/* Show Spreadsheet link to the left of the connection badge */}
            {sheetConfig.spreadsheetId && (
              <a
                href={`https://docs.google.com/spreadsheets/d/${sheetConfig.spreadsheetId}`}
                target="_blank"
                rel="noreferrer"
                className="px-3 py-1.5 rounded-xl border border-slate-200 text-[11px] font-semibold text-emerald-700 bg-emerald-50/15 hover:bg-emerald-50/40 transition-colors flex items-center gap-1.5 cursor-pointer shadow-3xs"
              >
                <span>打开 Google 电子表格 ↗</span>
              </a>
            )}
      ...
      <main className="max-w-[1600px] mx-auto p-4 sm:p-6 w-full flex-1 space-y-6">
              ...
              <div className="bg-white border border-slate-200/90 rounded-2xl shadow-xs p-5 space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs text-slate-600 font-semibold">
                  ...
                  <form onSubmit={handleSaveConfig} className="md:col-span-2 space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-slate-100 pt-3">
                      <div className="text-[10px] text-gray-400 flex items-center gap-1 font-medium font-sans">
                ...
                {sheetConfig.syncMode === 'appsScript' && (
                  <div className="bg-emerald-50/20 border border-emerald-100/60 p-3.5 rounded-xl space-y-2">
                    <p className="flex items-center justify-between text-xs text-emerald-950 font-bold font-sans">
        ...
        {/* 1.5 Top Summary Statistics Dashboard Row */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-in">
          {/* Card 1: Total Invoices */}
          <div style={{ marginTop: '-15px', marginLeft: '0px', marginBottom: '15px' }} className="bg-gradient-to-br from-indigo-50/40 via-white to-white border border-indigo-100 rounded-2xl p-3 flex flex-col justify-between shadow-3xs hover:shadow-2xs transition-all">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-extrabold text-indigo-800 uppercase tracking-wider">
                总票据
              </span>
              <div className="p-1.5 bg-indigo-50 border border-indigo-150 text-indigo-755 rounded-lg shrink-0">
                <FileSpreadsheet size={13} />
              </div>
            </div>
            <div className="mt-1">
              <p className="text-lg sm:text-xl font-black text-slate-900 font-mono">
                {totalInvoicesCount} <span className="text-[10px] text-gray-400 font-bold font-sans">单</span>
              </p>
            </div>
          </div>

          {/* Card 2: Pending PO */}
          <div style={{ marginLeft: '0px', marginTop: '-15px', marginBottom: '15px' }} className="bg-gradient-to-br from-amber-50/30 via-white to-white border border-amber-100 rounded-2xl p-3 flex flex-col justify-between shadow-3xs hover:shadow-2xs transition-all">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-extrabold text-amber-800 uppercase tracking-wider">
                待 PO 票据
              </span>
              <div className="p-1.5 bg-amber-50 border border-amber-100/80 text-amber-650 rounded-lg shrink-0">
                <FileText size={13} />
              </div>
            </div>
            <div className="mt-1">
              <p className="text-lg sm:text-xl font-black text-slate-900 font-mono">
                {pendingPoCount} <span className="text-[10px] text-slate-450 font-black font-sans">单</span>
              </p>
            </div>
          </div>

          {/* Card 3: Pending Request Payment */}
          <div style={{ marginTop: '-15px', marginBottom: '15px' }} className="bg-gradient-to-br from-sky-50/30 via-white to-white border border-sky-100 rounded-2xl p-3 flex flex-col justify-between shadow-3xs hover:shadow-2xs transition-all">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-extrabold text-sky-800 uppercase tracking-wider">
                待提付款
              </span>
              <div className="p-1.5 bg-sky-50 border border-sky-100/80 text-sky-655 rounded-lg shrink-0">
                <Receipt size={13} />
              </div>
            </div>
            <div className="mt-1">
              <p className="text-lg sm:text-xl font-black text-slate-900 font-mono">
                {pendingRequestPaymentCount} <span className="text-[10px] text-slate-450 font-black font-sans">单</span>
              </p>
            </div>
          </div>

          {/* Card 4: Pending payout */}
          <div style={{ marginTop: '-15px', marginBottom: '15px' }} className="bg-gradient-to-br from-rose-50/30 via-white to-white border border-rose-100 rounded-2xl p-3 flex flex-col justify-between shadow-3xs hover:shadow-2xs transition-all">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-extrabold text-rose-800 uppercase tracking-wider">
                待付款
              </span>
              <div className="p-1.5 bg-rose-50 border border-rose-100/80 text-rose-650 rounded-lg shrink-0">
                <CreditCard size={13} />
              </div>
            </div>
            <div className="mt-1">
              <p className="text-lg sm:text-xl font-black text-slate-900 font-mono">
                {pendingPaidCount} <span className="text-[10px] text-slate-450 font-black font-sans">单</span>
              </p>
            </div>
          </div>
        </div>

        {/* 2. Primary Layout: Two-Column Workspace Flexbox */}
        <div className="flex flex-col lg:flex-row gap-6 items-start">

          {/* LEFT COLUMN: Compact Upload Box and Parser state */}
          <div className="w-full lg:w-[200px] lg:shrink-0 space-y-4">
            <div style={{ paddingLeft: '15px', paddingRight: '15px', paddingTop: '15px', paddingBottom: '15px', marginLeft: '0px', marginRight: '0px', marginTop: '-30px' }} className="bg-white border border-slate-200/90 rounded-3xl shadow-3xs space-y-4">
              <div className="border-b border-slate-100 pb-2.5">
                ...
                <p className="text-[10px] text-gray-400 mt-0.5">上传新 PDF 原始账单、拍照对账</p>
              </div>

              {/* Drag & Drop File Container Zone (Redesigned to a smaller size) */}
              <div
                ...
                className={`border-2 border-dashed rounded-2xl p-4 sm:p-5 py-6 sm:py-7 transition-all flex flex-col items-center justify-center text-center cursor-pointer relative group overflow-hidden ${
                  dragOver 
                    ? 'border-emerald-500 bg-emerald-50/10 shadow-inner' 
                    : 'border-slate-200 hover:border-emerald-450 bg-slate-50/20 hover:bg-white shadow-3xs'
                }`}
              ...
              >
                ...
                <div className="hidden md:flex flex-col items-center justify-center">
                  <div className="p-3 bg-white group-hover:bg-emerald-50 border border-slate-150 group-hover:border-emerald-100 rounded-xl text-slate-400 group-hover:text-emerald-700 transition-all mb-2 shadow-3xs">
                    <UploadCloud size={18} className="animate-bounce" />
                  </div>

                  <h3 className="text-[11px] font-bold text-slate-800 tracking-tight">
                    点击或拖放发票文件至此
                  </h3>

                  <div className="mt-2 flex flex-wrap justify-center gap-1">
                    <span className="text-[8px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-150 px-1.5 py-0.5 rounded">
                      PDF
                    </span>
                    <span className="text-[8px] font-bold bg-blue-50 text-blue-700 border border-blue-150 px-1.5 py-0.5 rounded">
                      PNG
                    </span>
                    <span className="text-[8px] font-bold bg-purple-50 text-purple-700 border border-purple-150 px-1.5 py-0.5 rounded">
                      JPEG
                    </span>
                  </div>
                </div>
                ...
                <div className="flex md:hidden flex-col items-center justify-center w-full space-y-3">
                  <div className="grid grid-cols-2 gap-2 w-full">
                    ...
                    <button
                      ...
                      className="p-3 bg-emerald-50/60 hover:bg-emerald-50 border border-emerald-150 rounded-xl flex flex-col items-center justify-center gap-1 transition-all text-emerald-900 active:scale-98 cursor-pointer"
                    >
                      <UploadCloud size={14} className="text-emerald-600 animate-pulse" />
                      <span className="text-[10px] font-black leading-none">上传 PDF/图片</span>
                    </button>
                    ...
                    <button
                      ...
                      className="p-3 bg-indigo-50 hover:bg-indigo-100/80 border border-indigo-150 rounded-xl flex flex-col items-center justify-center gap-1 transition-all text-indigo-900 active:scale-98 cursor-pointer"
                    >
                      <Camera size={14} className="text-indigo-600" />
                      <span className="text-[10px] font-black leading-none">手机拍照模式</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            {isParsing && (
              <div className="bg-white border border-slate-200/90 rounded-2xl p-5 text-center space-y-3 shadow-3xs">
                ...
                <div className="space-y-1">
                  <h4 className="text-[11px] font-bold text-slate-900 tracking-tight font-sans">AI 扫描分析大脑连线中...</h4>
                  <p className="text-[10px] text-emerald-800 font-semibold px-2 min-h-[30px] leading-relaxed">
            ...
            {/* Parser Error Block */}
            {parserError && (
              <div className="bg-rose-50 border border-rose-150 rounded-2xl p-4 flex items-start gap-2 text-rose-800 text-xs shadow-3xs">
              ...
              {activeDraft && (
                <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-5 overflow-hidden" id="review-modal-portal">
                  <motion.div
                    ...
                    className="relative bg-white border border-slate-200/95 rounded-3xl overflow-hidden shadow-2xl max-w-6xl w-full max-h-[92vh] flex flex-col animate-fade-in"
                    id="review-modal-box"
                  >
                    {/* Top Right Floating Close Button */}
                    <button
                      onClick={handleCancelAllDrafts}
                      className="absolute top-4 right-4 z-55 p-1 px-2 border border-rose-200 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-full transition-all cursor-pointer text-xs font-bold font-mono shadow-xs"
                      title="关闭并取消所有草稿核对"
                    >
                      ✕ 取消
                    </button>
                    ...
                    {/* Meta Header */}
                    <div className="p-4 pr-16 sm:p-5 sm:pr-20 border-b border-slate-100 bg-slate-50/45 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shrink-0">
                      <div className="flex items-center gap-2.5">
                      ...
                      <div className="flex flex-wrap items-center gap-2 w-full md:w-auto shrink-0 justify-end">
                        ...
                        <button
                          onClick={handleCancelAllDrafts}
                          className="flex-1 md:flex-none p-2 px-3 text-xs bg-rose-600 hover:bg-rose-700 text-white rounded-xl transition-colors font-extrabold border border-rose-600 shadow-xs cursor-pointer flex items-center gap-1 justify-center whitespace-nowrap"
                          title="放弃修改并关闭"
                        >
                    ...
                    <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 select-text">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50/50 p-4 border border-slate-100 rounded-2xl">
                        ...
                        {/* Supplier */}
                        <div className="flex flex-col">
                          <label className="text-[9px] font-bold text-gray-400 uppercase mb-1">供应商</label>
                          <input
                            type="text"
                        ...
                        <table className="w-full text-left text-xs border-collapse min-w-[850px]">
                          ...
                          <tbody className="divide-y divide-slate-100 text-slate-700 bg-white">
                              ...
                              <tr key={item.id} className="hover:bg-slate-50/35 transition-colors">
                                ...
                                <td className="py-1 px-3 text-right">
                                  <input
                                    ...
                                    onChange={(e) => handleItemFieldChange(item.id, 'totalPrice', Number(e.target.value))}
                                    className="w-full bg-slate-50/50 hover:bg-slate-50 border border-slate-200 focus:border-emerald-550 rounded-lg p-1 text-right font-mono text-xs font-bold text-slate-800"
                                    required
                                  />
                                </td>
                            ...
                            <tr className="bg-slate-50/50 border-t border-slate-100">
                              <td colSpan={6} className="py-2 px-3 text-right text-slate-500 font-bold text-[10.5px]">
                                📑 整单不含税金额 (可直接修改)：
                              </td>
                              <td className="py-1 px-3 text-right">
                                <input
                                  type="number"
                                  step="any"
                                  value={draftUntaxedTotal === 0 ? '' : draftUntaxedTotal}
                                  onChange={(e) => handleDraftTotalAmountChange(Number(e.target.value))}
                                  className="w-full bg-amber-50/30 hover:bg-amber-50 border border-amber-200 focus:border-amber-550 rounded-lg p-1 text-right font-mono text-xs font-black text-slate-800 focus:ring-1 focus:ring-amber-500"
                                />
                              </td>
                              <td></td>
                            </tr>
                            ...
                            <tr className="bg-slate-50/50">
                              ...
                              <td className="py-1 px-3 text-right">
                                <input
                                  type="number"
                                  step="any"
                                  value={draftTax === 0 ? '' : draftTax}
                                  onChange={(e) => handleDraftTaxAmountChange(Number(e.target.value))}
                                  className="w-full bg-amber-50/30 hover:bg-amber-50 border border-amber-200 focus:border-amber-550 rounded-lg p-1 text-right font-mono text-xs font-bold text-amber-700 focus:ring-1 focus:ring-amber-500"
                                />
                              </td>
                              <td></td>
                            </tr>
                            ...
                            <tr className="bg-emerald-50/15 border-t-2 border-emerald-100 font-extrabold text-slate-900">
                              <td colSpan={6} className="py-3 px-3 text-right text-emerald-950 font-black text-[11px]">
                                💰 含税金额合计 (可直接修改)：
                              </td>
                              <td className="py-1.5 px-3 text-right">
                                <input
                                  type="number"
                                  step="any"
                                  value={draftTaxInclusive === 0 ? '' : draftTaxInclusive}
                                  onChange={(e) => handleDraftTaxInclusiveChange(Number(e.target.value))}
                                  className="w-full bg-emerald-50 hover:bg-emerald-100 border border-emerald-250 focus:border-emerald-500 rounded-lg p-1 text-right font-mono text-xs font-black text-emerald-700 focus:ring-1 focus:ring-emerald-500"
                                />
                              </td>
            ...
            <div style={{ marginTop: '-30px', paddingLeft: '10px', paddingRight: '10px', paddingTop: '15px', paddingBottom: '15px', width: '1275.4px' }} className="flex-1 bg-white border border-slate-200/90 rounded-3xl shadow-xs space-y-5" id="local-ledger-board">
              <div style={{ marginBottom: '5px', paddingBottom: '0px', paddingRight: '0px', paddingTop: '0px' }} className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {/* Compact Search Bar placed next to the actions */}
                  <div className="relative w-40 sm:w-48">
                    ...
                    <input
                      ...
                      placeholder="搜索金额/日期/物料..."
                      className="w-full pl-7.5 pr-2 py-1.5 border border-slate-200 rounded-lg text-[10.5px] bg-slate-50/40 text-slate-800 focus:bg-white outline-hidden font-medium"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 p-1.5 px-2.5 text-[11px] bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 font-extrabold border border-slate-200 rounded-xl transition-all cursor-pointer disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 p-1.5 px-2.5 text-[11px] bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-xl transition-all cursor-pointer shadow-2xs"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 p-1.5 px-2.5 text-[11px] bg-slate-50 hover:bg-slate-105 border border-slate-200 rounded-xl transition-colors font-semibold text-slate-650 cursor-pointer"
                  >
                    <Download size={10} />
                    <span>空录模板</span>
                  </button>
              ...
              {sheetFetchError && (
                <div className="bg-rose-50 border border-rose-150 rounded-2xl p-4 space-y-2 mt-2 select-text text-slate-800">
                  ...
                  {(sheetFetchError.includes("doGet") || sheetFetchError.toLowerCase().includes("apps script") || sheetFetchError.includes("HTTP 500") || sheetFetchError.includes("HTTP")) && (
              ...
              {sheetReceipts.length === 0 ? (
                ...
                <div style={{ paddingLeft: '0px', paddingRight: '0px' }} className="border border-slate-200/90 rounded-2xl overflow-hidden bg-white max-h-[580px] overflow-y-auto overflow-x-auto shadow-2xs">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="bg-slate-50 text-slate-500 font-extrabold border-b border-slate-200/90 text-[10px]">
                        ...
                        <th className="py-2.5 px-3 w-[390px] text-center">操作</th>
                      </tr>
                    </thead>
                          ...
                          >
                            ...
                            <td className="py-2 px-3 text-center">
                              <div className="flex items-center justify-center gap-1.5 flex-wrap">
                                {/* Modern Segmented Tracking Switch Capsule */}
                                <div className="inline-flex items-center bg-slate-100/80 rounded-full p-0.5 border border-slate-200/60 shadow-3xs select-none gap-0.5 mr-0.5 shrink-0">
                                  <button
                                    onClick={() => handleToggleTrackingStatus(record.id, 'po')}
                                    className={`px-1.5 py-0.5 text-[9.5px] rounded-full transition-all font-bold cursor-pointer flex items-center gap-0.5 whitespace-nowrap ${
                                      trackingStatuses[record.id]?.po
                                        ? 'bg-amber-500 text-white shadow-xs font-black'
                                        : 'text-slate-400 hover:text-slate-700 bg-transparent opacity-65 hover:opacity-100'
                                    }`}
                                    title="追踪流程：PO 已关联"
                                  >
                                    <FileText size={9} />
                                  ...
                                  <button
                                    onClick={() => handleToggleTrackingStatus(record.id, 'request_payment')}
                                    className={`px-1.5 py-0.5 text-[9.5px] rounded-full transition-all font-bold cursor-pointer flex items-center gap-0.5 whitespace-nowrap ${
                                      trackingStatuses[record.id]?.request_payment
                                  ...
                                  <button
                                    onClick={() => handleToggleTrackingStatus(record.id, 'paid')}
                                    className={`px-1.5 py-0.5 text-[9.5px] rounded-full transition-all font-bold cursor-pointer flex items-center gap-0.5 whitespace-nowrap ${
                                      trackingStatuses[record.id]?.paid
            ...
            >
              ...
              <div className="p-6 overflow-y-auto space-y-5 text-xs">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  ...
                  <div className="p-3 bg-slate-50 border border-slate-150 rounded-xl space-y-1">
                    <span className="text-[9px] font-bold text-gray-400 uppercase">供应商</span>
              ...
              <div className="p-4 px-6 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      const success = handleLoadRecordToDraft(selectedRecordForDetail);
                      if (success) {
                        setSelectedRecordForDetail(null);
                      }
                    }}
                    className="p-2 px-4 rounded-xl text-xs bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold shadow-2xs cursor-pointer flex items-center gap-1"
                  >
                    <Edit size={12} />
                    <span>编辑此单</span>
                  </button>
